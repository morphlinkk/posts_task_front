        ...
        <div class="relative flex flex-col w-full h-full text-gray-700 bg-white shadow-md rounded-lg bg-clip-border">
            <table class="w-full text-left table-fixed min-w-max">
                ...
                <tbody>
                    ...
                    <template v-else>
                        <tr v-for="post in store.currentPagePosts" :key="post.id" class="hover:bg-slate-50 border-b border-slate-200 h-16">
                            <PostTemplate :post="post" />
                        </tr>
                    </template>
                </tbody>
            </table>
            <div class="flex justify-between items-center px-4 py-3" v-if="store.totalPostsCount">
                ...
                <div class="flex space-x-1">
                    ...
                    <button @click="() => goToPage(page)" v-for="page in store.totalPages" :key="page" class="px-3 py-1 min-w-9 min-h-9 text-sm font-normal text-white bg-slate-800 border border-slate-800 rounded hover:bg-slate-600 hover:border-slate-600 transition duration-200 ease">
                        {{ page }}
                    </button>
...
const displayEndIndex = computed(() => {
    const calculatedIndex = (store.page*store.limit)
    return (calculatedIndex>store.totalPostsCount) ? store.totalPostsCount : calculatedIndex
});
